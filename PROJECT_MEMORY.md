...
- `GITHUB_TOKEN`으로 만든 auto-commit push는 새 workflow run을 트리거하지 않는다.
- 따라서 schedule 또는 `workflow_dispatch`에서 변경이 생기면 같은 run 안에서 이어서 build/deploy가 수행된다.
...
### 4. build job은 importer가 만든 최신 branch 상태를 다시 checkout해야 함

- importer는 auto-commit을 수행할 수 있다.
- 따라서 build는 `github.sha`가 아니라 최신 branch ref를 checkout해야 importer 결과를 포함할 수 있다.
...
- 현재 구조에서는 auto-commit이 생겨도 `GITHUB_TOKEN` push만으로 별도 workflow run이 자동 생성되지는 않는다.
- 따라서 같은 run 안에서 importer 후 build/deploy까지 이어지면 정상이다.
- 최종적으로 `[Auto] Notion posts update` 커밋이 1번 생기고 해당 run이 성공하면 정상이다.
